...
fn test_beads_integration_with_question() {
    ...
    // Check stderr for bead creation message (may or may not succeed depending on bd availability)
    let stderr = String::from_utf8_lossy(&output.stderr);
    // Either bead was created or warning about bd not being available
    assert!(
        stderr.contains("Created bead issue") ||
        stderr.contains("Warning: Failed to create bead") ||
        stderr.is_empty(),
        "Expected bead creation message or warning, got: {}",
        stderr
    );
}
...
fn test_beads_disabled_by_default() {
    ...
    // Should not have any bead-related messages in stderr
    let stderr = String::from_utf8_lossy(&output.stderr);
    assert!(!stderr.contains("Created bead issue"),
        "Beads should not be enabled by default");
}
