...
mod tests {
    use super::*;
    #[test] fn test_ralph_integration_creation() { let ralph = RalphTuiIntegration::new(false, None); assert!(!ralph.enabled); }
    #[test] fn test_ralph_task_id_extraction() { let ralph = RalphTuiIntegration::new(true, Some("tasks/example-task.md".to_string())); assert_eq!(ralph.task_id, Some("example-task".to_string())); }
    #[test] fn test_ralph_default() { let ralph = RalphTuiIntegration::default(); assert!(!ralph.enabled); }
}
