...
impl ExperimentOrchestrator {
    pub fn new(cli: Cli) -> Self {
        ...
        let beads = if cli.beads_enabled { Some(BeadsIntegration::new(true)) } else { None };
        let ralph = if cli.ralph_tui_enabled { Some(RalphTuiIntegration::new(true, cli.ralph_task_file.clone())) } else { None };
        Self { cli, session_manager, metric_evaluator, beads, ralph }
    }
    ...
    fn get_question(&self) -> Result<String> {
        if let Some(ref ralph) = self.ralph {
            if let Ok(Some(question)) = ralph.read_task_from_file() { return Ok(question); }
        }
        if let Some(ref q) = self.cli.question { return Ok(q.clone()); }
    ...
    fn generate_experiment_design(&self, question: &str) -> ExperimentDesign {
        ...
        if let Some(ref metric) = self.cli.metric { design.metric = metric.clone(); }
        if let Some(ref measure) = self.cli.measure { design.measurement = measure.clone(); }
        if let Some(baseline) = self.cli.baseline { design.baseline = baseline; }
        if let Some(target) = self.cli.target_improvement { design.target_improvement = target; }
        design
    }
    ...
    async fn verify_baseline_only(&self) -> Result<()> {
        let metric = self.cli.metric.clone().ok_or_else(|| anyhow::anyhow!("--metric is required for baseline verification"))?;
        let measurement = self.cli.measure.clone().ok_or_else(|| anyhow::anyhow!("--measure is required for baseline verification"))?;
    ...
    async fn resume_experiment(&self, session_id: &str) -> Result<()> {
        let session = self.session_manager.find_session(session_id)?.ok_or_else(|| anyhow::anyhow!("Session '{}' not found", session_id))?;
        println!("Resuming session: {}", session_id);
        let resume_baseline = session.iterations.iter().filter(|it| it.kept).map(|it| it.metric_value).fold(session.baseline_record.value, |min, val| min.min(val));
        ...
        for iter in &result.iterations {
            let _ = self.session_manager.save_iteration(iter);
        }
        ...
        let finalization_result = finalization_executor.finalize(&merged_session, target_improvement, result.stuck_reason.as_ref())?;
        self.print_finalization_result(&finalization_result, target_improvement);
        if finalization_result.success { Ok(()) } else { std::process::exit(1); }
    }
    async fn run_full_experiment(&mut self, question: &str, design: ExperimentDesign) -> Result<()> {
        let baseline_record = if let Some(baseline) = self.cli.baseline {
            ...
            crate::phase1_design::BaselineRecord::new(chrono::Utc::now().to_rfc3339(), git_commit, design.metric.clone(), design.measurement.clone(), baseline, vec![baseline, baseline], 0.0, true)
        } else {
            let baseline_result = self.metric_evaluator.verify_baseline(&design.metric, &design.measurement)?;
            ...
            baseline_result.baseline_record.ok_or_else(|| anyhow::anyhow!("No baseline record available"))?
        };
        ...
        let mut session = ExperimentSession::new(session_id, question.to_string(), design.clone(), baseline_record.clone());
        for iter in &result.iterations { session.add_iteration(iter.clone()); }
        session.finalize(result.best_iteration, if result.best_metric < baseline_record.value { "completed".to_string() } else { "no_improvement".to_string() });
        self.session_manager.save_session(&session)?;
        let target_improvement = self.cli.target_improvement.unwrap_or(design.target_improvement);
        let finalization_executor = FinalizationExecutor::new();
        let finalization_result = finalization_executor.finalize(&session, target_improvement, result.stuck_reason.as_ref())?;
        if let Some(ref mut beads_integration) = self.beads {
            let _ = beads_integration.close_bead(finalization_result.success, finalization_result.final_improvement, session.iterations.len());
        }
        if let Some(ref ralph_integration) = self.ralph {
            let _ = ralph_integration.mark_task_complete(finalization_result.success, finalization_result.final_improvement, session.iterations.len());
        }
        self.print_finalization_result(&finalization_result, target_improvement);
        if finalization_result.success { Ok(()) } else { std::process::exit(1); }
    }
    ...
    fn print_finalization_result(&self, result: &crate::phase3_merge::FinalizationResult, _target_improvement: f64) {
        if self.cli.quiet { return; }
        eprintln!("\n=== Finalization ===");
        if result.success {
            eprintln!("✓ Target improvement achieved: {:+.2}%", result.final_improvement * 100.0);
            if let Some(ref branch) = result.branch_name { eprintln!("✓ Created branch: {}", branch); }
            if let Some(ref msg) = result.commit_message { eprintln!("\nCommit message:\n{}", msg); }
        } else {
            self.print_failure_report(result);
        }
    }
    fn print_failure_report(&self, result: &crate::phase3_merge::FinalizationResult) {
...
fn read_question_from_stdin() -> Result<String> {
    ...
    if question.is_empty() { anyhow::bail!("Question cannot be empty"); }
    Ok(question)
}
