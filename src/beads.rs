...
mod tests {
    use super::*;
    #[test] fn test_beads_integration_creation() { let beads = BeadsIntegration::new(false); assert!(!beads.enabled); }
    #[test] fn test_beads_default() { let beads = BeadsIntegration::default(); assert!(!beads.enabled); }
}
